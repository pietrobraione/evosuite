...
public class SimilarityWithRefToFreshObjectSubTypes extends SimilarityWithRef {
	private static final Logger logger = new Logger(SimilarityWithRefToFreshObjectSubTypes.class);

	private final Class<?> theReferredClass;
	
	public SimilarityWithRefToFreshObjectSubTypes(String theReferenceOrigin, Class<?> theReferredClass) {
	...
	protected double evaluateSimilarity(CandidateBackbone backbone, Object referredObject) {
		...
		final double samePackageSimilarity = 0.0d;
		final double sameClassSimilarity = 0.0d;
		...
		try {
			theReferredClass.cast(referredObject);
